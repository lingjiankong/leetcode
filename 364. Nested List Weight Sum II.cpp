...
// After the max depth is known (which is sumAtAllLevels.size()), we add the corresponding weight to the values in
// sumAtAllLevels and add all numbers together
...
private:
    void preorder(NestedInteger& root, int depth, vector<int>& sumAtAllLevels) {
        ...
        if (root.isInteger()) {
            sumAtAllLevels[depth] += root.getInteger();
            return;
        }
