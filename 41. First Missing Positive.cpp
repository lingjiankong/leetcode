...
// Example:
//
// Given (after sorted) [-4, 0, 1, 2, 4, 5, 6, 100]
//
// [0 1 2 3 4 5 6 7]
...
int firstMissingPositive(vector<int>& nums)
{
	...
	for (int i = 0; i < nums.size(); ++i)
	{
		...
		// but the new nums[i] might still be incorrect. We must have WHILE loop here instead of an IF statement.
		// For example, if we have [3, 1, 4, -1], when i = 0, we swap nums[0] with nums[3-1] and get [4, 1, 3, -1].
		...
		{
			// put nums[i] at the place it belongs to (nums[nums[i]-1])
	...
	for (int i = 0; i < nums.size(); ++i)
	{
		// e.g. [1,99,3,4], 99 is not 2, then 2 is the first missing positive
		if (nums[i] != i + 1)
		{
			return i + 1;
		}
