...
//
// In the second for loop, you iterate from right to left, and make sure if left child has higher rating than right child,
// he gets more candy than right child.
//
// It is possible that left child already has many candies
// (for example in the case of [1, 2, 3, 4, 5, 1], the child with rating of 5 already has 5 candies
// when we scanned from left to right),
// thus we want candies[i] = max(candies[i], candies[i+1] + 1).
//
