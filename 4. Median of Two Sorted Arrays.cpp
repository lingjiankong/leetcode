...
double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
    ...
    while (left <= right) {
        ...
        // R1 too small, pushing up the range to search cut1 to the right by left = cut1 + 1
        if (L2 > R1) {
            left = cut1 + 1;
        }
        // L1 too big, pulling down the range to search cut1 to the left by right = cut1
        else if (L1 > R2) {
            right = cut1;
        }
