...
// timestamp 2, then the only value is at timestamp 1 is "bar". timeMap.set("foo", "bar2", 4); // store the key "foo"
// and value "ba2r" along with timestamp = 4. timeMap.get("foo", 4);         // return "bar2" timeMap.get("foo", 5); //
// return "bar2"
//
//
// Constraints:
...
public:
    ...
    void set(string key, string value, int timestamp) { _map[key][timestamp] = value; }

    string get(string key, int timestamp) {
        if (not _map.count(key)) {
            return "";
        }

        auto itr = _map[key].upper_bound(timestamp);

        // In this case, all timestamps are greater than target timestamp.
        if (itr == _map[key].begin()) {
            return "";
        }
