...
class TicTacToe {
public:
    TicTacToe(int n) { _board.resize(n, vector<int>(n, 0)); }

    int move(int row, int col, int player) {
        _board[row][col] = player;
        int i = 0, j = 0, n = _board.size();

        // Check current column and see if win.
        for (i = 1; i < n; ++i) {
            if (_board[i][col] != _board[i - 1][col]) {
                break;
            }
        }
        if (i == n) {
        ...
        for (j = 1; j < n; ++j) {
            if (_board[row][j] != _board[row][j - 1]) {
                break;
            }
        }
        if (j == n) {
        ...
        if (row + col == n - 1) {
            for (i = 1; i < n; ++i) {
                if (_board[n - i - 1][i] != _board[n - i][i - 1]) {
                    break;
                }
            }
            if (i == n) {
