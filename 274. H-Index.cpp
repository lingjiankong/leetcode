...
public:
    int hIndex(vector<int>& citations) {
        // By definition on Wikipedia, we can find H-index in the following way: sort the array (contains the number of citations for each paper) in descending order, give each a index start from 1. From left to right, find the last number >= its index, the result is its index. In the case below, the H-index is 3.
    ...
    int hIndex2(vector<int>& citations) {
        ...
        int n = citations.size();
        vector<int> papers (n+1, 0); // the jth element is the number of papers with j citations, e.g. there are papers[j] number of papers with j citations

        for (const int& citation : citations)
            ++papers[citation > n ? n : citation]; // max possible H-index is the number of paper published (n), thus papers with number of citations greater than n is set to have n citations
