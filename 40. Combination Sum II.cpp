...
// 2. Sort the input array and check if (i > startIndex && candidates[i-1] == candidates[i]), this prevent duplicate result, for example, [1, 7] and [7, 1] will only appear once.
//
    ...
    private:
        ...
        void backtrack(vector<int>& candidates, int target, int startIndex, vector<int>& current, vector<vector<int>>& all)
        {
            ...
            for (int i = startIndex; i < candidates.size(); ++i)
            {
                ...
                backtrack(candidates, target - candidates[i], i + 1, current, all);
                current.pop_back();
            }
        }
