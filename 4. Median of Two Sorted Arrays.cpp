...
double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2)
{
	...
	if (n1 == 0)
	{
		return (double)(nums2[(n2-1)/2] + nums2[n2/2]) / 2;
	}
	...
	while (cut1 <= n1)
	{
		...
		// L1 too big, pulling down the range to search cut1 to the left by cutR = cut1 - 1
		if (L1 > R2)
		{
			cutR = cut1 - 1;
		}
		// R1 too small, pushing up the range to search cut1 to the right cutL = cut1 + 1
		else if (L2 > R1)
		{
			cutL = cut1 + 1;
		}
		...
		// that's why we return median of sorted array as (double)(max(L1, L2) + min(R1, R2)) / 2.
		//
		...
		// thus we return median of sorted array as (double)(min(R1, R2)).
		else
		{
			if (len % 2 == 0)
			{
