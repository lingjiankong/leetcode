...
int firstMissingPositive(vector<int>& nums)
{
	...
	for (int i = 0; i < nums.size(); ++i)
	{
		...
		//
		// For example, if we have [3, 1, 4, -1], when i = 0, num[0] = 3, we swap nums[0] with nums[3-1] and get [4, 1, 3, -1].
		// Now, nums[2] = 3, which is correct. However, nums[0] = 4, if we were using an if loop, nums[0] still has the wrong value,
