...
// Using a hash map, but you sort of implement it yourself
bool isIsomorphic(string s, string t) {
    // 256 because there are 256 ASCII characters. -1 is just arbitrary. You can initialize the numbers to -123 if you
    // want. The idea is that, if letter a in mapS maps to letter b in mapT, then, letter a and letter b must share the
    // same "value". This "value" is the 'i' we assign to mapS[a] and mapT[b].
