...
// content. You need to output 1. Example 2:
//
// Input: g = [1,2], s = [1,2,3]
// Output: 2
...
public:
    int findContentChildren(vector<int>& g, vector<int>& s) {
        // first of all, sort greed factor and cookie size
        sort(g.begin(), g.end());
        sort(s.begin(), s.end());

        int count = 0;  // total number of content children
        int j = 0;      // give the jth cookie to ith child

        // iterate through greed factor of each child
        for (int i = 0; i < g.size(); ++i) {
            // in this case, jth cookie cannot satisfy ith child, try next cookie (++j)
            while (j < s.size() and g[i] > s[j]) {
                ++j;
            }

            // in this case, jth cookie can satisfy ith child.
            // ++count and move cookie pointer to the next cookie for next iteration.
            if (j < s.size()) {
                ++count;
                ++j;
            }
