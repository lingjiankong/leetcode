...
// this problem can be done via dp, there are four states we keep in our dp, see comments in code for what these four states represent. You need to keep in mind that all these states keep track of the MAX AMOUNT OF MOENY we have in each state.
//
...
public:
    int maxProfit(vector<int>& prices) {
        ...
        int buy1 = INT_MIN; // max amount of money we have after we've bought the first stock
        int sell1 = 0; // max amount of money we have after we've sold the first stock (occurs after first stock has been bought)
        int buy2 = INT_MIN; // max amount of money we have after we've bought the second stock (occurs after the first stock has been sold)
        int sell2 = 0; // max amount of money we have after we've sold the second stock (occurs after second stock has been bought)
        for (auto price : prices) {
            sell2 = max(sell2, buy2 + price);
            buy2 = max(buy2, sell1 - price);
            sell1 = max(sell1, buy1 + price);
