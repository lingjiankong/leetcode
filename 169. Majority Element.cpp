...
// 2b. Moore voting algorithm: The idea is to repeatedly get rid of pairs of different element,
...
int majorityElement(vector<int>& nums) {
    // What majorityElement equal to in the beginning doesn't matter, it is just an arbitrary guess.
    ...
    for (int num : nums) {
        if (num == majorityElementCandidate) {
            ...
            // Hence we set the new majorityElement to current num, all subsequent iterations compare num with current
            // majorityElement. If they are different, then we get rid of a pair of (majorityElement, num) and --count,
            // else we get another majorityElement and ++count.
            majorityElementCandidate = num;
            count = 1;
        } else {
            // In this case, nums != majorityElement, thus we get rid of one pair of different number (majorityElement,
            // num)
            --count;
        }
    }
